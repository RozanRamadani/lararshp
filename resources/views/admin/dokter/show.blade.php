    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <div class="flex items-center space-x-4">
                ...
                <a href="{{ route('admin.dokter.edit', $dokter->iduser) }}" class="bg-teal-600 hover:bg-teal-700 text-white px-4 py-2 rounded-lg transition-colors">
                    Edit
                </a>
            </div>
        </div>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
                <div class="p-6">
                    <h3 class="text-lg font-semibold mb-4 text-gray-800">Informasi Dokter</h3>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label class="block text-sm font-medium text-gray-500">Nama Lengkap</label>
                            <p class="mt-1 text-base text-gray-900">{{ $dokter->nama }}</p>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-500">Email</label>
                            <p class="mt-1 text-base text-gray-900">{{ $dokter->email }}</p>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-500">Alamat</label>
                            <p class="mt-1 text-base text-gray-900">{{ $dokter->dokter->alamat ?? '-' }}</p>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-500">Nomor HP</label>
                            <p class="mt-1 text-base text-gray-900">{{ $dokter->dokter->no_hp ?? '-' }}</p>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-500">Bidang Dokter</label>
                            <p class="mt-1 text-base text-gray-900">{{ $dokter->dokter->bidang_dokter ?? '-' }}</p>
                        </div>
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-base text-gray-900">
                                @if($dokter->dokter)
                                    {{ $dokter->dokter->jenis_kelamin == 'L' ? 'Laki-laki' : 'Perempuan' }}
                                @else
                                    -
                                @endif
                            </p>
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-base text-gray-900">{{ $dokter->no_wa ?? '-' }}</p>
                        </div>
                        @endif
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-base text-gray-900">{{ optional($dokter->created_at)->format('d M Y') ?? '-' }}</p>
                        </div>
                    </div>
                </div>
            </div>
